...
On 2026-09-18 the user requested removing the first-line status heading from task activity cards. Version 0.3.5 shows the native status icon and task text on one line for added, started and completed activities, independent of language. The status remains accessible to screen readers. Count-only task creation cards keep their sole heading; blank task text also restores the heading.

The user then approved applying the same one-line presentation to Search and Task notification: native icon plus summary, with the heading retained only when no summary is supplied. Version 0.3.6 preserves accessible action names and native disclosure/file behavior.
