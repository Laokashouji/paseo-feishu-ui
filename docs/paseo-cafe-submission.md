...
Validation on 2026-09-14: the community's `scripts/validate-registry.ts` passed with live GitHub checks (`1 registry entry validated OK`). Validator source revision: `f5d390c50a9bc2dcae4250010b5551f76c866c60`. Submitted on 2026-09-14 as [paseo-cafe/paseo-cafe#115](https://github.com/paseo-cafe/paseo-cafe/pull/115), based on upstream `ead6547`, pointing at the public 0.3.2 release. Live registry validation, app formatting/typecheck/build, and 104 companion-plugin tests passed. One root test failure reproduced with the new entry removed; the full-repo formatting check also found a pre-existing `.release-please-manifest.json` issue. Both baseline findings are disclosed in the PR. Submission is awaiting review; it is not an accepted listing yet.
