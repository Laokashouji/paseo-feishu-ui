# Paseo 0.9.1 compatibility — 0.3.8, 2026-09-23

- Before the change, the local 0.9.1 daemon rejected the plugin because its requirement ended at `<0.9.0`; the desktop had no skin styles despite retaining the selected Feishu theme ID. DevBox remained on 0.8.0.
- Expanded the range to `>=0.8.0 <0.10.0` and pinned development SDK types and their matching peers to 0.9.1. The used client contracts remain compatible; no new 0.9-only API is required.
- Typechecks against both SDK 0.8.0 and 0.9.1, native-entry smoke tests and the browser regression pass. The browser checks exercise both themes at 1342px/390px, card interactions, boundaries, node reuse, theme isolation and multi-owner cleanup. On macOS Paseo 0.9.1 in the dark theme, the adapter recognized all 29 mounted tool badges and 32 response frames. The native mixed-response regression passed at 1920×1050 and 700×850, with aligned bubble edges and no virtual-gap/footer crossing. Chats had equal client/scroll widths (1268px and 700px); command headers measured 46px/52px with zero icon/text/header center offsets. The native composer retained its original control and adapter marker. Screenshots were reviewed at both sizes. Disabling removed all owned nodes, markers and the document registry; re-enabling restored the skin. The original window size was restored. Native light-theme switching was not completed because the user navigated back to a conversation during the check; both themes are covered by browser tests. Native iOS/Android hardware and permission/file interactions remain unverified.
...
- Typecheck, native-entry smoke tests and browser regression pass. The browser regression first failed on the two-line layout, then passed with added/started/completed labels in Chinese and English, light/dark themes at 1342px/390px, equal icon/text/header centers, empty-text fallback, native expansion/focus and existing lifecycle checks. Synthetic screenshots were reviewed.
- In installed macOS Paseo 0.8.0, the reported conversation's added and started headers changed from 56px to 46px. Added/started/completed task previews all measured zero icon/text/header center difference and remained middle segments of the shared response. Existing added/started header nodes retained identity. The active chat measured 1268px for both client and scroll width. Its current mounted history had 174 rows; two Task notification cards still used two lines. The temporary verification restored the tab active immediately before the check and did not write a draft or scroll the conversation.
- An audit across both desktop windows also found Search headers using two lines. Counts vary as timelines stream and virtualized rows mount. See [two-line card audit](two-line-card-audit.md) for the remaining categories. Native light/compact rendering, task-header interaction and iOS/Android hardware were not exercised; light/compact and interaction coverage above is from the browser fixture.
